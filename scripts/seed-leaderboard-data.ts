  ...
  try {
    ...
    for (const userData of usersData) {
      ...
      for (let i = 0; i < userData.totalSessions; i++) {
        const daysAgo = Math.floor(Math.random() * 60); // Random day in the past 60 days
        const sessionDate = dayjs().subtract(daysAgo, 'days');
        const startTime = sessionDate.hour(Math.floor(Math.random() * 12) + 8); // Random hour between 8-20
        const sessionDuration = 30 + Math.floor(Math.random() * 60); // 30-90 minutes
