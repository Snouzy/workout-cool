...
export function ShareWorkoutButton({ startedAt, muscles, exercisesCount }: ShareWorkoutButtonProps) {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      if (!blob) {
        console.error("Failed to generate workout share image: empty blob");
        return;
      }
      ...
      if (typeof navigator !== "undefined" && navigator.canShare?.({ files: [file] })) {
        await navigator.share({
          ...
          text: t("workout_builder.session.share_card_motivation"),
        });
      } else {
  ...
  return (
    <>
      <Button className="gap-2" disabled={isGenerating} onClick={handleShare} variant="secondary">
        {isGenerating ? <Loader2 className="h-4 w-4 animate-spin" /> : <Share2 className="h-4 w-4" />}
        {t("workout_builder.session.share_my_success")}
      </Button>

      {/* Off-screen source for html-to-image; keep measurable (not display:none). */}
