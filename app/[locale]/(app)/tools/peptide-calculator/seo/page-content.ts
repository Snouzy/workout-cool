...
const ru: PeptidePageContent = {
  ...
  sections: [
    {
      ...
      body: [
        ...
        "Прежде чем набирать, посмотрите на схему шприца. Число единиц — это отметка, по которой вы выравниваете поршень; число в миллилитрах — та же величина в единицах, напечатанных на цилиндре туберкулинового шприца. Оба числа описывают один и тот же объём.",
      ],
    },
    {
      id: "formula",
      heading: "Формула разведения пептидов",
      lead: "Разведение пептида считается в два шага, и каждый из них — деление: концентрация равна массе пептида, поделённой на объём воды, а объём дозы равен дозе, поделённой на концентрацию. Флакон на 10 мг, разведённый 5 мл бактериостатической воды, даёт 2 мг/мл, поэтому доза 250 мкг занимает 0,125 мл.",
      body: [
        ...
        "Число доз во флаконе выводится из тех же двух чисел: весь пептид поделить на дозу. 10 мг — это 10 000 мкг, а 10 000 поделить на 250 — получается 40 доз. Это быстрая проверка на здравый смысл: если калькулятор показывает во флаконе на 10 мг всего три дозы по 250 мкг, что-то введено неверно.",
      ],
    },
    {
      id: "chart",
      ...
      body: [
        ...
        "Ни одна строка этой таблицы не говорит, сколько пептида брать. В каждой строке общая масса до и после смешивания одна и та же: вода меняет концентрацию и объём набора, но не количество пептида во флаконе. Доза приходит от врача, который её назначил, или из инструкции к препарату, — таблица лишь показывает, что получается из конкретного разведения.",
      ],
    },
    {
      id: "conversion",
      heading: "Перевод мкг в единицы инсулинового шприца",
      lead: "Микрограммы переводятся в единицы инсулинового шприца одной формулой: разделите дозу в мкг на концентрацию в мг/мл, а результат — ещё на десять. При 2 мг/мл доза 250 мкг — это 250, поделённые на 20, то есть 12,5 единицы. Та же доза при 10 мг/мл — 2,5 единицы.",
      body: [
        "Десятка берётся из шприца, а не из пептида. Одна единица на шприце U-100 — это 0,01 мл, поэтому 0,125 мл и есть 12,5 единицы. Умножить объём в миллилитрах на 100 — способ такой же надёжный, как формула выше.",
        "Концентрация и единицы движутся в противоположные стороны. Удвойте концентрацию — единиц станет вдвое меньше, потому что та же масса пептида уложена в половину жидкости. Поэтому в одной строке таблицы перевода стоят сначала 12,5 единицы, потом 5, потом 2,5: одна доза, три разведения, три разные отметки на цилиндре.",
        "Прежде чем верить любому числу единиц, посмотрите, какой шприц у вас в руке. Шприц U-100 размечен на 100 единиц в миллилитре, шприц U-40 — на 40 единиц в миллилитре, и те же 0,125 мл дадут на нём 5 единиц. Единица — это шкала, напечатанная на цилиндре, а не постоянное количество.",
      ],
    },
    {
      id: "u100",
      ...
      body: [
        ...
        "Маленький цилиндр читается легче. На шприце объёмом 0,3 мл деления стоят дальше друг от друга, и 12,5 единицы попадает отчётливо между двумя штрихами, а не жмётся к ним. Если рассчитанный объём туда влезает, маленький шприц обычно даёт более точный набор.",
    ...
    },
    {
      ...
      lead: "Бактериостатическая вода — это стерильная вода с 0,9 % бензилового спирта, консерванта, который сдерживает рост бактерий и позволяет прокалывать флакон не один раз. В стерильной воде консерванта нет, поэтому после прокола крышки она уже не защищает содержимое от загрязнения между двумя наборами.",
      body: [
        "На арифметику не влияет ни та, ни другая. 5 мл — это 5 мл, и флакон на 10 мг с любой из них даёт 2 мг/мл. Выбор определяет, сколько разведённый флакон останется пригодным и как его хранить, а не объём, который вы набираете.",
        ...
        "Вливайте воду медленно по внутренней стенке флакона, а не струёй на порошок, и дайте флакону постоять, пока раствор не станет прозрачным. Не встряхивайте. Пептиды — хрупкие молекулы, и взбалтывание разрушает их, ничего не меняя в том, что считает калькулятор.",
      ],
    },
    {
      id: "mistakes",
      heading: "Частые ошибки в расчёте дозы пептида",
      lead: "Большинство ошибок в расчёте дозы пептида — ошибки единиц измерения, и они огромные: путаница микрограммов с миллиграммами сдвигает запятую в 1 000 раз, а шприц U-40, прочитанный как U-100, искажает объём в 2,5 раза. И то и другое даёт число, которое на цилиндре выглядит правдоподобно.",
      body: [
        ...
        "Ещё две мелочи проверьте отдельно. Лиофилизированный порошок сам занимает немного объёма, поэтому итоговой жидкости может оказаться чуть больше, чем воды, которую вы влили, — калькулятор этого не учитывает. И пептид, размеченный в международных единицах, а не в миллиграммах, не подставить в расчёт по мг без коэффициента пересчёта именно для этого препарата.",
      ],
    },
    {
      id: "faq",
  ...
  conversionTable: {
    ...
    headers: ["Доза", "при 2 mg/mL", "при 5 mg/mL", "при 10 mg/mL"],
    rows: [
      ["250 mcg", "12.5 U", "5 U", "2.5 U"],
      ["500 mcg", "25 U", "10 U", "5 U"],
      ["1000 mcg", "50 U", "20 U", "10 U"],
...
export const PEPTIDE_CALCULATOR_CONTENT: Partial<Record<Locale, PeptidePageContent>> = { en, es, fr, pt, ru };
