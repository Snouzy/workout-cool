...
interface ExerciseSelectionProps {
  ...
  onSelectExercise: (exercise: ExerciseForStatistics) => void;
}

const MUSCLES = [
  "CHEST", "BACK", "SHOULDERS", "BICEPS", "TRICEPS", "LEGS", "ABDOMINALS"
];
...
export const ExerciseSelection: React.FC<ExerciseSelectionProps> = ({
  ...
  const handleSelectForStats = useCallback((exercise: ExerciseForStatistics) => {
    onSelectExercise(exercise);
    onOpenChange(false);
  }, [onSelectExercise, onOpenChange]);

  const renderExerciseItem = useCallback((exercise: ExerciseForStatistics) => {
    const primaryMuscle = exercise.attributes.find(attr => attr.attributeName.name === "PRIMARY_MUSCLE")?.attributeValue.value;
    
    return (
      <div className="flex items-center gap-4 p-4 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700" key={exercise.id}>
        <div 
        ...
        <Button
          className="shrink-0"
          onClick={() => handleSelectForStats(exercise)}
          size="sm"
          variant="outline"
        >
  ...
  const renderContent = useCallback(() => {
    ...
    return (
      <div className="space-y-4">
        {data.data.map(renderExerciseItem)}
      </div>
    );
