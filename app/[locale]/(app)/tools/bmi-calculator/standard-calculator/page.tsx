...
import { BmiCalculatorClient } from "../shared/BmiCalculatorClient";
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return generateSEOMetadata({
    title: "Standard BMI Calculator - Body Mass Index Calculator",
    description:
      "Calculate your BMI (Body Mass Index) with our standard calculator. Get instant results with health category and personalized recommendations.",
    keywords: ["BMI calculator", "body mass index", "weight status", "health assessment", "BMI chart"],
    locale,
    ...
    structuredData: {
      ...
      calculatorData: {
        ...
        targetAudience: ["general public", "health conscious individuals", "fitness enthusiasts"],
        relatedCalculators: ["adjusted-calculator", "pediatric-calculator", "bmi-comparison"],
      },
    },
  });
}

export default async function StandardBmiCalculatorPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getI18n();
  ...
  return (
    <>
      <SEOScripts
        canonical={`${getServerUrl()}/${locale}/tools/bmi-calculator/standard-calculator`}
        description="Calculate your BMI (Body Mass Index) with our standard calculator. Get instant results with health category and personalized recommendations."
        locale={locale}
        structuredData={{
          type: "Calculator",
          calculatorData: {
