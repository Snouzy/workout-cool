...
const zhCN: PeptidePageContent = {
  heroSubtitle: "填入西林瓶用量、你加入的抑菌水体积和目标剂量，得到精确的毫升数，以及 U-100 胰岛素注射器上抽取至哪一格刻度。",
  sections: [
    {
      id: "how-to",
      heading: "多肽计算器怎么用",
      lead: "多肽计算器只要三个数字：西林瓶标签上的多肽毫克数、你加进去的抑菌水毫升数，以及你要抽取的剂量微克数。三个都填好，它给出对应的毫升体积，以及 U-100 胰岛素注射器上抽取至哪一格刻度。",
      body: [
        ...
        "剂量最后填，写成 mcg，照处方或产品标签上的数字填。计算器不评判这个数字，它只做换算。2 mg/mL 下的 250 mcg 剂量是 0.125 mL，在 U-100 注射器上是 12.5 单位，一支 10 mg 的西林瓶装得下 40 次这样的剂量。",
        "抽之前先看注射器示意图。单位数是你对齐推杆的那一格；毫升数是同一个量，换成结核菌素注射器针筒上印的那种单位。两个数字说的是同一个体积。",
      ],
    },
    {
      id: "formula",
      ...
      body: [
        "除之前先把质量统一成一个单位，不然整个算式塌掉。微克和毫克差 1000 倍：250 mcg 是 0.25 mg，1000 mcg 是 1 mg。两者混用一次，抽出来的体积就差一千倍。",
        "把这个例子一步步走完。10 mg 多肽除以 5 mL 水是 2 mg/mL。250 mcg 剂量是 0.25 mg。0.25 mg 除以 2 mg/mL 是 0.125 mL。这个毫升数就是答案，后面所有换算只是换刻度，不是换量。",
        "每瓶剂量数出自同样两个数：总多肽量除以单次剂量。10 mg 是 10000 mcg，10000 除以 250 得 40 次。这个数字顺手能当校验用——如果计算器说 10 mg 的瓶只有三次 250 mcg 的剂量，你填的数里有一项错了。",
      ],
    },
    {
      id: "chart",
      ...
      body: [
        "按你想量的体积挑一行，不要按哪个数看着整挑。浓度高，每次注射的液体少，瓶里搁着的水也少；浓度低，同一个剂量摊在更多格刻度上，小剂量更容易读准。",
    ...
    },
    {
      ...
      lead: "微克换算成胰岛素注射器单位只用一条式子：单位数等于剂量微克数除以浓度 mg/mL 的十倍。2 mg/mL 下，250 mcg 剂量是 250 除以 20，等于 12.5 单位。同一个剂量在 10 mg/mL 下是 2.5 单位。",
      body: [
        "那个十倍来自注射器，不来自多肽。U-100 注射器上一个单位是 0.01 mL，所以 0.125 mL 就是 12.5 单位。把毫升数乘 100，跟上面那条式子一样可靠。",
        "浓度和单位反着走。浓度翻倍，单位数减半，因为同样质量的多肽挤进了一半的液体里。所以换算表的一行里会依次出现 12.5、5 和 2.5 单位：一个剂量，三种配比，针筒上三个不同的位置。",
        "信任何单位数之前，先确认手里是哪种注射器。U-100 注射器一毫升是 100 单位；U-40 注射器一毫升是 40 单位，同样的 0.125 mL 在它上面是 5 单位。一个单位是针筒上那套刻度里的一格，不是一个固定的量。",
      ],
    },
    {
      id: "u100",
      heading: "U-100 注射器为什么一毫升是 100 单位",
      lead: "U-100 是胰岛素的浓度标准：每毫升 100 国际单位。针筒照这个标准印刻度，100 单位刚好填满 1 mL，一个单位就是 0.01 mL。换成复溶好的多肽，这套刻度不带任何胰岛素含义，它只是一把量百分之一毫升的细尺。",
      body: [
        ...
        "半单位的刻度不是每支都印。有些针筒只印整单位，这时 12.5 单位得在两格之间估，或者改配比让数字正好落在印出来的线上。先看你自己那支注射器，再去信计算器暗示的精度。",
      ],
    },
    {
      id: "water",
    ...
    },
    {
      id: "mistakes",
      heading: "多肽剂量计算的常见错误",
      lead: "多肽剂量算错，多数是单位错，而且错得很大：微克和毫克搞混，小数点挪了 1000 倍；把 U-40 注射器当成 U-100 来读，体积差 2.5 倍。这两种错误在针筒上看起来都像个正常数字。",
      body: [
        "接下来是以为加水量是定好的。10 mg 的西林瓶没有标准加水量——2 mL 得 5 mg/mL，5 mL 得 2 mg/mL——所以从别人瓶子上抄来的单位数，在你的瓶上是错的。按你实际加进去的体积重算。",
        "换了配比还沿用旧数字，是同一个错误的慢镜头。上一瓶 250 mcg 是 12.5 单位，这一瓶也是 12.5 单位，只在浓度完全相同时成立。每复溶一瓶就重算一次。",
        "另外两处小的值得查一下。冻干粉自己也占一点体积，最后的液体可能比你加进去的水略多一些，而计算器假设它不占；还有，用国际单位而不是毫克标注的多肽，没有那个产品专用的换算系数，填不进按 mg 算的计算器。",
      ],
    },
    {
      id: "faq",
      heading: "常见问题",
      lead: "关于多肽复溶、抑菌水加多少、以及胰岛素注射器单位的常见问题，下面逐条回答。每个回答走的都是计算器的那两步：多肽质量除以加水体积得到浓度，剂量除以浓度得到体积。",
      body: [],
    },
  ],
  reconstitutionTable: {
  ...
  faq: [
    ...
    },
    {
      ...
      answer:
        "U-100 指针筒照每毫升 100 单位的浓度印刻度，所以一个单位是 0.01 mL，100 单位填满 1 mL。U-40 注射器印的是每毫升 40 单位。用错的那套刻度读体积，会差 2.5 倍。",
    },
  ],
